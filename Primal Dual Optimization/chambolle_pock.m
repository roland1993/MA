...
%   tol1    ~ 1 x 1                 tolerance for duality gap decrease
%   tol2    ~ 1 x 1                 tolerance for primal objective decrease
%   tol3    ~ 1 x 1                 tolerance for dual objective decrease
% OUT:
...
while (i < maxIter) && ...
        ((i < 4) || ...
            1 - (primal_history(i) / primal_history(i - 3)) > tol2) %&& ...
%         ((i < 4) || ...
%             1 - (primal_history(i) - dual_history(i)) / ...
%             (primal_history(i - 3) - dual_history(i - 3)) > tol1) && ...
%         ((i < 4) || ...
%             (dual_history(i) / dual_history(i - 3)) - 1 > tol3)
    ...
    % iterative output
    if abs(dual_history(i + 1)) == inf
        s1 = '      ';
    else
        s1 = '';
    end
    fprintf('%d\t%.2e\t%.4f\t\t %.2e%s\t%.4f\t\t %.2e\n', i, ...
        primal_history(i + 1), ...
...
elseif (1 - (primal_history(i) - dual_history(i)) / ...
        (primal_history(i - 3) - dual_history(i - 3)) <= tol1)
    fprintf(...
        'RELATIVE GAP DECREASE OVER LAST 3 ITERATES %.2e <= %.2e', ...
        1 - (primal_history(i) - dual_history(i)) / ...
        (primal_history(i - 3) - dual_history(i - 3)), tol1);
    
elseif (1 - (primal_history(i) / primal_history(i - 3)) <= tol2)
    fprintf(...
        'RELATIVE PRIMAL DECREASE OVER LAST 3 ITERATES %.2e <= %.2e', ...
        1 - (primal_history(i) / primal_history(i - 3)), tol2);
    
elseif ((dual_history(i) / dual_history(i - 3)) - 1 <= tol3)
    fprintf(...
        'RELATIVE DUAL INCREASE OVER LAST 3 ITERATES %.2e <= %.2e', ...
        (dual_history(i) / dual_history(i - 3)) - 1, tol3);
